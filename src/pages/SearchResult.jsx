...
function SearchResult() {
    ...
    useEffect(() => {
        ...
        sectionRef.current?.scrollTo({ top: 0, behaviour: 'smooth' });

        dispatch(getAllVideosByOptions({ search: query, page: 1, limit: 6 }))
            .unwrap()
            .then(() => {
    ...
    const handleScroll = () => {
        ...
        if (scrolledValue + 5 > scrollHeight) {
            ...
            if (
                ...
                dispatch(
                    getAllVideosByOptions({ search: query, page: `${currentPagingInfo.nextPage}`, limit: 6 })
                );
            }
        }
    };
