...
import { getAllVideos } from "../../app/Slice/videoSlice";
...
function ChannelVideos({ owner = false }) {
    ...
    return videos?.length < 1 ? (
        ...
        <ul
            className={`grid grid-cols-[repeat(auto-fit,_minmax(300px,_1fr))] ${videos?.length < 3 && "lg:grid-cols-[repeat(auto-fit,_minmax(300px,0.25fr))]"
                } gap-4 pt-2`}
        >
            {videos.map((video) => (
